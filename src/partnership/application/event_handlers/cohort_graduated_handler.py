"""CohortGraduatedHandler — ACL that bridges CohortGraduated event to Partnership."""
...
class CohortGraduatedHandler:
    """Anti-Corruption Layer: handles CohortGraduated events from cohort_learning.

    Responsibilities:
    1. Read cohort data (module_id, cohort_size, curator list, helper metrics,
       avg review score) from the cohort_learning UoW.
    2. Compute curator_score per curator from HelperMetrics.
    3. Compute avg_review_score for the cohort from peer reviews.
    4. Call CalculateCurationCommissionUseCase once per curator.

    The handler opens the cohort_learning UoW in read-only mode and delegates
    all partnership writes to the use case (which owns the partnership UoW).
    """
    ...
    def handle(self, event: CohortGraduated) -> None:
