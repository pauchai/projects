"""Tests for ActivatePracticeTaskUseCase — Draft → Active workflow."""
...
class TestActivatePracticeTask:
    """Test suite for activating a practice task (Draft → Active)."""

    def test_activates_draft_task(self) -> None:
        """Master can activate a draft task."""
        # Arrange
    ...
    def test_curator_can_activate_task(self) -> None:
        """Module curator can activate a draft task."""
    ...
    def test_raises_when_task_already_active(self) -> None:
        """Raises ValueError when task is already active."""
        # Arrange
        cohort = make_cohort(cohort_id="c1", master_id="master1")
        task = make_task(task_id="t1", cohort_id="c1", creator_id="master1")
        task.activate()  # Manually activate to test idempotency
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="Cannot transition from active to active"):
            use_case.execute(task_id="t1", caller_id="master1")

    def test_raises_when_task_closed(self) -> None:
        """Cannot activate a closed task."""
        # Arrange
        cohort = make_cohort(cohort_id="c1", master_id="master1")
        task = make_task(task_id="t1", cohort_id="c1", creator_id="master1")
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="Cannot transition from closed to active"):
            use_case.execute(task_id="t1", caller_id="master1")
