/**
 * Cohort workspace — Dashboard tab (master only).
 * Pending competency validations + curator promotions.
 * Ported from cohort-dashboard.tsx.
 */
...
export function CohortDashboardPage() {
  ...
  if (cohortLoading) return <p className="text-muted-foreground">Loading dashboard…</p>
  if (!cohort) return <p className="text-destructive">Cohort not found.</p>

  const isMaster = cohort.master_id === userId
  if (!isMaster) {
    return <p className="text-destructive">Only the cohort master can access this dashboard.</p>
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-bold">Cohort Dashboard</h2>
      ...
      <section>
        <h3 className="text-lg font-semibold mb-3">Pending Competency Validations</h3>
        {validationsLoading && <p className="text-muted-foreground text-sm">Loading…</p>}
        {validationsError && (
      ...
      <section>
        <h3 className="text-lg font-semibold mb-3">Pending Curator Promotions</h3>
        {promotionsLoading && <p className="text-muted-foreground text-sm">Loading…</p>}
        {promotionsError && (
